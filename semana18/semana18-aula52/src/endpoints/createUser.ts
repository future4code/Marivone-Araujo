...
export default async function createUser(
  ...
  try {
    ...
    if (!name || !nickname || !email || !password || !role || !cep || !number || !adicional_Info) {
      res.statusCode = 422;
      throw new Error("Preencha os campos 'name','nickname', 'password', 'email', 'role', 'cep', 'number', 'adicional_Info'")}
    if (role !== userRole.admin && role !== userRole.normal) {throw new Error("Os valores possíveis para 'role' são 'normal' e 'admin'");
    }
    ...
    const newUser: user = {
      id, name, nickname, email, password: cypherText, role, cep, number, adicional_Info};
    ...
    const userAddress = {
      cep, street, number, adicional_Info, neighbourhood, city, state, user_id: id};
